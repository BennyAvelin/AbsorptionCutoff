...
in every fixed positive dimension the rounded squared-radius chain is absorbed at
the origin, both in the sense that the survival probability of the time-`t`
marginal tends to zero and that the absorption time is almost surely finite.
...
* `Q₁`, unit-grid rounding: `AbsorptionCutoff/Rounding.lean`;
...
* `roundedRadiusBound` and the theorem: `AbsorptionCutoff/Metastability.lean`.
