...
/-!
# Mathlib-only challenge: fixed-dimensional almost-sure absorption

This file is the comparator challenge surface for the fixed-dimensional clause of
the paper's rounded-metastability theorem `thm:rounded-qualitative-metastability`:
under the theorem's metastability-threshold hypothesis, in every fixed positive
dimension the rounded vector chain is absorbed at the origin almost surely from
every deterministic rounded initial condition.

It imports only `Mathlib`. Every object the statement mentions is rebuilt below
from Mathlib primitives, so a reader can check the claim without reading — or
trusting — the 70k-line development. The source correspondences are:

* `Q₁`, `gridRound`, `Qρ`: `AbsorptionCutoff/Rounding.lean`;
* `roundedExistenceThreshold`: `AbsorptionCutoff/Metastability.lean`;
* `gaussianMat`, `Pstep`: `AbsorptionCutoff/VectorReduction.lean`;
...
* the theorem: `AbsorptionCutoff/Metastability.lean`.
