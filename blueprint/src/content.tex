...
\begin{definition}[Total-variation cutoff]
  ...
  \leanok
  \uses{def:dseq}
  For a family indexed by $n\to\infty$ with cutoff time $t_n$ and window $a_n$,
  the windowed-convention cutoff of the paper's Definition.
\end{definition}
...
\begin{theorem}[Rounded qualitative metastability and absorption]
  ...
  The rounded chain remains in a compact subset of the rightmost positive-drift
  component for an exponentially long time with probability tending to one as
  the dimension grows, uniformly over the prescribed compact initial set.
  In each fixed dimension, the same chain is absorbed at zero almost surely,
  and its survival probability tends to zero. These are the two clauses of the
  paper's qualitative metastability theorem.
\end{theorem}
\begin{proof}\leanok\end{proof}
...
\begin{theorem}[Supercritical dimension cutoff]
  ...
  \lean{AbsorptionCutoff.exists_stationary_family_hasCutoff_of_forall_mem_Icc,
    ...
    AbsorptionCutoff.exists_unique_invariant_probability_Kchain_of_apply_singleton_zero}
  \leanok
  \uses{def:Kchain, prop:gaussian-compactness-selection,
  ...
  Then the origin-free invariant scalar laws selected for $K_{A,N}$ satisfy
  the paper's two iterated total-variation limits at the cutoff center
  $t_N(q_0)$ with constant window one.  The interior irreducibility theorem
...
\begin{corollary}[Vector supercritical cutoff]
  ...
  vector chain has the same cutoff center and constant window one.
\end{corollary}
\begin{proof}\leanok\end{proof}
...
\begin{theorem}[Fixed-width cutoff and mixing times]
  ...
  The rounded vector kernels have total-variation cutoff at center $L/\mu$ and
